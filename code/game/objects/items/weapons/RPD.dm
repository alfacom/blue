...
/obj/item/weapon/rpd
	...
	var/working = 0
	var/mode = 1
	var/list/modes = list("Atmos & Disposal Pipes","Deconstruct pipe")
	var/canRwall = 0
	var/disabled = 0
	var/p_type = -1
	var/d_p_type = -1
...
<A href='?src=\ref[src];makemeter=1;dir=1'>Meter</A><BR>
...
/obj/item/weapon/rpd/attack(mob/M as mob, mob/user as mob)
	if (M == user)
		switch (mode)
			...
			if (2)
				user << "<span class='notice'>deconstruction not working right now, use matter decompiler</span>"	//TODO
